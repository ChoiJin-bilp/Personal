...
//方法：获取增减分钟数后的日期
function getDateTimeAddMunites(dateTime, addcnt) {
  var endDate = dateTime;
  try {
    var value = dateTime.getTime(); //将开始时间转为毫秒            
    value += addcnt * 60 * 1000; //将天数转换成毫秒后与开始时间相加得到结束时间的毫秒数         
    endDate = new Date(value);
  } catch (err) {}
...
//方法：仅获取时间字符串：日期变量
function getTimeStrByDateTime(dateTime) {
