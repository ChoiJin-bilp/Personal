...
Page({
  ...
  queryOrderDetail: function () {
    ...
    var status = 0;
    var linkNo = "0,1,2,3,4"
    switch (that.data.currentData) {
      case 0:
        status = 0;
        linkNo = "0,1,2,3,4,5,6,7"
        break;
      case 1:
        status = 1;
        break;
  ...
  toDelivery: function (e) {
    ...
    if (orderList.linkNo == 0) {
      if (orderList.cnt == 1) {
        wx.navigateTo({
          url: "../shoporders/shoporders?orderid=" + orderList.orderId
        });
      } else {
        wx.navigateTo({
          url: "../shoporders/shoporders?orderid=&payno=" + orderList.payNo
        });
      }
